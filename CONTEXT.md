...
_Regenerated 2026-09-19T16:32:00+00:00 by `forecast/score.py`. Do not hand-edit._
...
- Observations: **58**
- Range: `2026-07-24` to `2026-09-19`
- Gaps: **0**
- Forecasts written: **52**
- Forecasts scored: **51**
- Awaiting outcome: **1**

Most recent scored call — `2026-09-19` (`model` mode): predicted **+1.16c**, actual **+0.72c**, error **-0.44c**.

## What the model is keying on

Standardised ridge coefficients, refit on all **50** training rows available today. Units are cents of predicted next-day change per one standard deviation of the feature, so the magnitudes are directly comparable to each other.

| feature | weight | what it is |
|---|---|---|
| `d1` | +1.057 | yesterday's change |
| `d2` | -0.503 | the change 2 days ago |
| `d3` | -0.254 | the change 3 days ago |
| `ma7` | +0.254 | mean of the last 7 daily changes |
| `wk` | +0.254 | 7-day change |
| `dow` | -0.202 | day of week (of the target) |
| `ma3` | +0.145 | mean of the last 3 daily changes |
| `vol7` | +0.084 | volatility of the last 7 daily changes |

Largest: `d1` at +1.057, 2.1× the next-largest (`d2`).
...
- Effective n, by error correlation: **20.0** (lag-1 r = -0.12)
- Effective n, by price-change correlation: **9.0** (lag-1 r = +0.38)
- **Gating on the lower: n_eff = 9.0** (the outcome figure).

- **Nothing is concludable at n_eff = 9.0.** The numbers below are recorded so the series exists, not because they support a claim. Do not quote them as skill.

| threshold | Brier | vs 0.25 | base rate | n |
|---|---|---|---|---|
| > -2c | 0.0027 | +0.2473 | 1.00 ⚠︎ degenerate | 20 |
| > -1c | 0.0248 | +0.2252 | 1.00 ⚠︎ degenerate | 20 |
| > +0c **(headline)** | 0.1138 | +0.1362 | 0.95 | 20 |
| > +1c | 0.1992 | +0.0508 | 0.55 | 20 |
| > +2c | 0.1881 | +0.0619 | 0.35 | 20 |
...
- PIT mean: **0.590** (target 0.500)
- 80% interval coverage: **80.0%** (target 80.0%)
- Residual sd 2.28c vs claimed sigma 1.57c (ratio 1.45)
- Spread check: **held** at n_eff = 9.0 (needs 20). Errors currently look wider than the claimed sigma, but that comparison is not yet evidence.
...
| 0.3–0.5 | 10 | 0.376 | 0.500 | +12.4pp |
| 0.5–0.7 | 16 | 0.595 | 0.750 | +15.5pp |
| 0.7–0.9 | 19 | 0.811 | 0.947 | +13.6pp |
